...
class UpcomingMoviesRepository(
    ...
    override fun createRemoteObservable(): Observable<List<MovieVo>> =
        remoteRepository.getMovies(DEFAULT_PAGE)
            .doOnNext {
                    ...
                    .doAfterSuccess { moviesId ->
                        localeRepository.saveCategoriesAndMovies(moviesId.map { id ->
                            MovieAndCategoryCrossRefEntity(
                                CategoryRepository.CategoryType.UPCOMING,
                                id
                            )
                        })
                            .applySchedulers()
    ...
    override fun createLocaleObservable(): Observable<List<MovieVo>> =
        localeRepository.getMovies(DEFAULT_PAGE)
}